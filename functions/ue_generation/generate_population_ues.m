function [UE_lats_flat, UE_lons_flat, Total_Pop] = generate_population_ues(lat_vec, lon_vec, people_per_ue)
    ...
    % Calculate Total Population and Target Number of UEs
    ...
    % Create the Weighted Probability Distribution
    ...
    % Convert the 1D linear indices back to 2D matrix rows/cols
    [rows, cols] = ind2sub(size(local_data), sampled_pixel_indices);
    
    % Convert the Rows/Cols back to real-world Latitudes and Longitudes
    [UE_lats_flat, UE_lons_flat] = intrinsicToGeographic(local_R, cols, rows);
    
    % (Optional) Add spatial Jitter 
