function [r0, v0] = generate_walker_states(orbit_height_m, inclination, ...
        planes, sats_per_plane, WalkerStar, phasing, min_elevation_deg, min_latitude_deg)
% GENERATE_WALKER_STATES  Frozen-orbit ECI initial states for Walker Delta/Star.
%
%   [r0, v0] = generate_walker_states(orbit_height_m, inclination, planes, ...
%                  sats_per_plane, WalkerStar, phasing, min_elevation_deg, min_latitude_deg)
%
%   Newton-solves the osculating eccentricity that makes every satellite share
%   the same inertial orbit shape under the RK4+J2 integrator, then places the
%   planes and in-plane phases according to the requested geometry:
%
%     WalkerStar = false (Walker Delta, i:T/P/F notation, T = planes*sats_per_plane)
%       RAAN spacing     = 360/planes              (uniform)
%       In-plane spacing = 360/sats_per_plane
%       Cross-plane phase= phasing * 360/total_sats
%
%     WalkerStar = true  (Walker Star)
%       RAAN spacing     = asymmetric seam-ratio (needs min_elevation_deg, min_latitude_deg)
%       In-plane spacing = 360/sats_per_plane
%       Cross-plane phase= in_plane_spacing/2      (brick-wall seam stagger)
%
...
%     WalkerStar        - logical; true for Walker Star, false for Walker Delta
%     phasing           - (Delta only) phasing factor F (integer, 0..P-1)
%     min_elevation_deg - (Star only) min UE elevation for seam-ratio geometry (deg)
%     min_latitude_deg  - (Star only) coverage reference latitude (deg); 0 = equatorial
%
%   Outputs
...
%
%   Examples
%     % GPS-like Walker Delta: 24 sats, 6 planes, F=1
%     [r0, v0] = generate_walker_states(20200e3, 55, 6, 4, false, 1);
%
%     % Polar Walker Star: 5 planes x 13 sats, 20 deg min elevation
%     [r0, v0] = generate_walker_states(1000e3, 90, 5, 13, true, 0, 20);
    ...
    arguments
        ...
        WalkerStar        (1,1) logical = false
        phasing           (1,1) double {mustBeInteger, mustBeNonnegative} = 0
        min_elevation_deg (1,1) double = 0
        min_latitude_deg  (1,1) double = 0
    end
    ...
    %% --- Plane RAAN spacing + in-plane phase stagger ---
    if WalkerStar
        % Asymmetric RAAN spacing (seam-ratio, same as generate_walker_star_scenario)
        Rs_km    = a / 1e3;
        ...
        delta_raan  = 180 / (planes - 1 + seam_ratio);   % co-rotating spacing
        delta_nu    = 360 / sats_per_plane;
        delta_phase = delta_nu / 2;                       % staggered "brick wall" seam
    else
        delta_raan  = 360 / planes;                       % uniform RAAN spacing
        delta_nu    = 360 / sats_per_plane;
