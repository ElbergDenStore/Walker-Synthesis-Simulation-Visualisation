function metrics = fast_coverage_simulator_function_v2(Cfg, reset_cache, calc_link, use_SGP, cancel_queue)
% FAST_COVERAGE_SIMULATOR_FUNCTION_V2
%
% Drop-in replacement for coverage_simulator_function / fast_coverage_simulator_function.
% Primary optimisation: the inner UE loop accumulates all results into pre-allocated
% plain matrices (double/int16) instead of writing into a struct array field-by-field.
% The UEs struct is built once from those matrices at the very end, only when needed
% (calc_link=true or an explicit caller requests SimData).
%
% Why this matters for gridsearch performance:
%   coverage_simulator_function writes  UEs(idx).SimData.X(has_service) = value
%   on every iteration.  Each such assignment triggers MATLAB's copy-on-write on the
%   struct entry, causing a heap allocation + memcopy for that field every iteration.
%   With 2146 UEs × 5 struct fields = 10730 heap allocs per Stage-3 call, and 32
%   workers competing for MATLAB's global allocator lock, this dominates wall time.
%   Plain matrix row-writes (best_el(idx,:) = ...) are in-place and lock-free.
%
% Signature is backward-compatible with coverage_simulator_function.m:
%   metrics = fast_coverage_simulator_function_v2(Cfg, reset_cache, calc_link, use_SGP)
%   metrics = fast_coverage_simulator_function_v2(Cfg, reset_cache, calc_link, use_SGP, cancel_queue)
%
% cancel_queue: optional parallel.pool.PollableDataQueue (same as coverage_simulator_function).

    if nargin < 5, cancel_queue = []; end

    fprintf('\n [v2] Fast Simulation: %d Sats, %.1f deg Inc\n', Cfg.Total_sats, Cfg.Inclination);
    tic
    ...
    if isfield(Cfg, 'DL')
        ...
        fprintf('\n [v2] Link budget complete (%.1f sec).\n', toc);
    end
